...
export default function CustomersManager() {
  ...
  useEffect(() => {
    axios
      .get("http://localhost/ecom-backend/auth/get-all-users.php")
      .then((res) => {
        if (res.data.status === 200) {
          const mappedUsers = res.data.data.map((user) => ({
            ...
            profile: "", // Agar DB me profile field ho to yaha map karo
            joinedDate: user.created_at.split(" ")[0],
            blocked: false,
          }));
